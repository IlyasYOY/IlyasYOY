...
Причем тут *jacoco*? *Jacoco* будет удовлетворен единственным тестом, что вызывает этот метод. Для него все ветки будут покрыты. Хотя она тут подразумевается не одна. Более того, тут может быть *NPE*, если `resource == null`. Наверно следует допустить, что это значение `NonNull`.

Я позволю себе немного отрефакторить код:
```java
    public static String fromResource(@NonNull AbstractFileResolvingResource resource) {
        ...
        if (resource.getFilename() == null) {
             ...
             // Хотя можно было бы и оставить вариант выше.
